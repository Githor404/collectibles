...
- **The measured costs — do not re-derive them by feel.** A suite run is **~12s**. A full 41-row defect pass is **~8.5 min**. A commit is **~10s**. On 2026-09-13 these were estimated at *"~250s per row"* — **wrong by 20×** — and it was the estimate, not the machine, that produced a day of avoidance: the full pass went unrun (leaving thirty rows unverified against changed code), invocations were sized to ~500s against a 580s ceiling and two background jobs were killed there, and work was batched into long uncommitted stretches to amortise a cost that did not exist. **A wrong cost estimate changed the working pattern, and the working pattern produced the losses.** Measure a number before letting it shape how the work is done.
