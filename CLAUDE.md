...
5. **A marketplace condition is never a comics grade** (D7). eBay's *Brand New / Like New / Very Good / Good / Acceptable* is a generic vocabulary that shares words with the grading scale and means different things. It splits **raw from slabbed** and nothing else; the seller's words show verbatim; no comp is assigned a grade its listing did not state. *(HT-D52's rule: when a scale is defined elsewhere, snap to its points or don't compute on it.)*
6. **A scatter is the claim** (D8). Sold comps are individual sales at whatever grades happened to sell — **no average, no midpoint, no "estimated value"**, no ladder synthesised from sparse data. Below 5 comps no range is shown; below 3 the surface says so.
...
**Identity comes from the Grand Comics Database** (free, official, JSON, no key) — R2a, pre-registered in `GATES.md`. **Price comes from eBay sold comps** via an Apify scraper, pay-per-lookup — R2b, not yet pre-registered. Sold comps are *actual sales*; everything below about PriceCharting describes *guide values*, and the two are different claims that must never share a label (brief rule 7's shape, applied to sources).
